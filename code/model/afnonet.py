...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=None, patch_size=8, in_chans=13, embed_dim=768):
        ...
        patch_size = (copy.deepcopy(patch_size), copy.deepcopy(patch_size))
...
class AFNONet(nn.Module):
    def __init__(self, img_size=None, patch_size=2, in_chans=20, out_chans=20, embed_dim=80, depth=3, mlp_ratio=4.,
        ...
        # 使用3D卷积，在时间维度上进行卷积，输入（N,C_in,time,H,W）即（N,C,2,H,W）->(N,C,1,H,W)->(N,C,H,W)
        self.conv3d = nn.Conv3d(in_channels=in_chans, out_channels=in_chans, kernel_size=(2, 3, 3), padding=(0, 1, 1))
        # self.batch2d = nn.BatchNorm2d(in_chans)

        self.embed_dim = embed_dim
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)  # python中的or返回第一个为True的值
        ...
        self.head = nn.Conv2d(out_chans * 4, out_chans, kernel_size=(2, 2))
        # self.head = nn.ConvTranspose2d(out_chans*4, out_chans, kernel_size=(2, 2), stride=(2, 2))   # , padding_mode='zeros'
        # self.head = F.conv_transpose2d(out_chans*4, out_chans, kernel_size=(2, 2), stride=(2, 2), padding_mode='replicate')
    ...
    def forward(self, x):
        ...
        # 使用3D卷积，在时间维度上进行卷积，输入（N,C_in,time,H,W）即（N,C,2,H,W）->(N,C,1,H,W)->(N,C,H,W)
        # print(x.shape)
        x = self.conv3d(x.permute(0, 2, 1, 3, 4)).squeeze(2)
        # x = self.batch2d(x)
        # print(x.shape)
        # 将x的维度由(N,C,161,161)转换为(N,C,162,162)
        # x = F.pad(x, (0, 1, 0, 1), mode='replicate')
        x = F.interpolate(x, size=(162, 162), mode='bilinear', align_corners=True)

        x = self.forward_features(x)
        # print(x.shape)
        x = self.final_dropout(x)
        # print('final_drop', x.shape)
        x = self.pre_logits(x)
        # print('pre_logits', x.shape)
        x = self.head(x)
        return x
...
if __name__ == '__main__':
    ...
    # net = AFNONet(img_size=[720, 1440], in_chans=20, out_chans=20, norm_layer=partial(torch.nn.LayerNorm, eps=1e-6))
    net1 = AFNONet(img_size=[162, 162], in_chans=70, out_chans=5)
    # 判读两个网络是否相同
    print('result shape: ', net1(torch.randn((48,2,70,161,161))).shape)    # for name, param in net.named_parameters():
