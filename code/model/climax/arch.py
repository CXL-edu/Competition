...
class ClimaX(nn.Module):
    ...
    def forward_encoder(self, x: torch.Tensor, lead_times: torch.Tensor, variables):
        ...
        lead_times = lead_times.unsqueeze(-1)
        # print(lead_times.shape, lead_times.dtype)
        lead_time_emb = self.lead_time_embed(lead_times)  # B, D
        # print(lead_time_emb.shape)
...
if __name__ == '__main__':
    # test model ClimaX
    model = ClimaX(default_vars=['t2m', 'tp', 'w10', 'mslp'])
    x = torch.randn(2, 4, 32, 64)
    lead_times = torch.tensor([0, 1], dtype=torch.float32)
    variables = ['t2m', 'tp', 'w10', 'mslp']
    out_variables = ['t2m', 'tp', 'w10', 'mslp']
    lat = torch.randn(32)
    preds = model(x, lead_times, variables, out_variables, lat)
    print(preds.shape)
