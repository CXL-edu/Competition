...
def get_region_info(img_size, patch_size):
    h, w = img_size
    patch_num = (h // patch_size) * (w // patch_size)
    patch_ids = list(range(patch_num))
    return {
        'patch_ids': patch_ids,
        'min_h': 0,
        'max_h': h - 1,
...
if __name__ == '__main__':
    # test get_region_info
    # lat = np.arange(-90, 90, 0.25)
    # lon = np.arange(0, 360, 0.25)
    import xarray as xr
    data = xr.open_dataset('../../constants_5.625deg.nc')
    lat, lon =  data['lat'].values, data['lon'].values
    # region_info = get_region_info('Global', lat, lon, 32)
    # print(region_info)
    # print(len(region_info['patch_ids']))
    region_info = get_region_info('NorthAmerica', lat, lon, 32)
    print(region_info)
    print(len(region_info['patch_ids']))
