...
class Model(ModelBase):
    def __init__(self):
        ...
        # encode
        self.conv0 = nn.Conv2d(ch[0], 64, 4, 1, 1)  # [B, 70, 161, 161] -> [B, 64, 160, 160]
        self.conv1 = nn.Conv2d(64, 64, 5, 2, padding=2)  # [B, 64, 160, 160] -> [B, 64, 80, 80]
        self.conv2 = nn.Conv2d(64, ch[1], 3, 1, padding=1)  # [B, 64, 80, 80] -> [B, 32, 80, 80]
        self.conv3 = nn.Conv2d(ch[1], ch[1], 5, 2, padding=2)  # [B, 32, 80, 80] -> [B, 32, 40, 40]
        self.conv4 = nn.Conv2d(ch[1], ch[2], 3, 1, padding=1)  # [B, 32, 40, 40] -> [B, 16, 40, 40]
        self.conv5 = nn.Conv2d(ch[2], ch[2], 5, 2, padding=2)  # [B, 16, 40, 40] -> [B, 16, 20, 20]
        self.conv6 = nn.Conv2d(ch[2], ch[3], 3, 1, padding=1)  # [B, 16, 20, 20] -> [B, 8, 20, 20]
        ...
        self.conv1_d = nn.Conv2d(8+16, 16, 3, 1, padding=1)  # cat(([B, 8, 40, 40], self.conv4(x)), axis=1) -> [B, 16, 40, 40]
        self.upsample2 = nn.Upsample(scale_factor=2, mode='bicubic', align_corners=True)  # [B, 16, 40, 40] -> [B, 16, 80, 80]
        self.conv2_d = nn.Conv2d(16+32, 24, 3, 1, padding=1)  # cat(([B, 16, 80, 80], self.conv2(x)), axis=1) -> [B, 24, 80, 80]
        self.upsample3 = nn.Upsample(scale_factor=2, mode='bicubic', align_corners=True)  # [B, 24, 80, 80] -> [B, 24, 160, 160]
        self.conv3_d = nn.Conv2d(24+64, 64, 3, 1, padding=1)  # cat(([B, 24, 160, 160], self.conv1(x)), axis=1) -> [B, 32, 160, 160]
        self.conv4_d = nn.Conv2d(64, 32, 5, 1, padding=2)  # [B, 32, 160, 160] -> [B, 5, 160, 160]
        self.conv5_d = nn.Conv2d(32, 16, 5, 1, padding=2)  # [B, 32, 160, 160] -> [B, 16, 160, 160]
        # 插值到161*161
        self.conv6_d = nn.Conv2d(17, 5, 5, 1, padding=2)  # cat([B, 16, 161, 161], time_emb) -> [B, 5, 161, 161]

        self.relu = nn.ReLU()
        # self.time_embed = nn.Embedding(20, 161*161)
        self.time_embed = nn.Linear(1, 161*161)
...
if __name__ == '__main__':
    ...
    time = torch.arange(0,20).repeat(8,1).view(-1,1).float()
